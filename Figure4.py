...
if PLOT_R:
    ...
    if PLOT_DATA:
        ...
        name = ['SAXS','SANS']
        color_data = ['red','blue']
        xtext,ytext,letter = -0.14,0.96,['A','B','C','D','E','F']
        for i in range(2):
    ...
    R_true = [10,30,50,70]
    minbin,maxbin = [0,22,41,67.5],[25,42,65,74]
    ...
    for k in range(len(R_true)):
        ...
        for w in [0,2,3,4]:
            for i in range(len(prior)):
                ...
                dev = abs(R-R_true[k])
                dev_stddev = np.std(dev)
                dev_error = dev_stddev/np.sqrt(len(R))
                R_dev2 = np.mean(abs(R-R_true[k]))
        ...
        ax[row,col].legend(frameon=False,title=r'weight ($w_j$)')
